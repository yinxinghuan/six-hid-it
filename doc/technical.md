...
- 角色身份：官网照片锁定黑白开脸特征；游戏主持人使用独立剪纸 SVG，官网版画头像只用于品牌依据和结算签章。眼白扩大为更宽的杏仁轮廓，虹膜使用 37×35 左右的椭圆，DOM 瞳孔使用 10×27 比例并保持原眼中心；两层胡须根部上移到鼻下嘴角附近。
...
- 猫表情与姿态：原 SVG 只保留眼白与虹膜；DOM 叠加的双瞳孔和双眉根据槽位、轻拨次数与揭晓阶段移动、收窄和下压。390px / 320px 画面分别使用 360px / 310px 宽的等比猫头；`data-phase` 控制 2.8 秒待机呼吸、放置与遮盖的 38–44px 下沉、1.4 秒洗杯跟踪和 1.9 秒选择观察起伏。`is-cat-emerging` 在选择开始时触发 520ms 探头，`is-probing` 在轻拨时额外上探 16px；揭晓继续复用蓄势、侧倾与碰撞姿态。
