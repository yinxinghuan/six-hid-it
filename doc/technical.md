...
- `_qa/capture-cat-motion.mjs`：在 390×844 与 320×568 正常动态模式下采集观察起伏、轻拨接触和恢复的连续帧，并记录猫头实际边界与位移。
- `_qa/capture-eye-tracking.mjs`：隐藏外部访客栏后，在 390×844 与 320×568 采集洗杯逐次扫视、选择居中、选左和改选右的 platform-layout 帧，记录瞳孔尺寸与实际 transform；另采集一张 external-guest 入口检查扩展栏。
- `_qa/verify-immediate-reveal.mjs`：监听答案杯与冰花的类名变化，验证杯子进入翻滚与冰花进入可见态发生在同一帧，并在离杯 70ms 后采集实机画面。

## 3. 核心模块

- 角色身份：官网照片锁定黑白开脸特征；游戏主持人使用独立剪纸 SVG，官网版画头像只用于品牌依据和结算签章。眼白扩大为更宽的杏仁轮廓，虹膜使用 37×35 左右的椭圆，DOM 瞳孔使用 12×29 比例并保持原眼中心；两层胡须根部上移到鼻下嘴角附近。
...
- 猫表情与姿态：原 SVG 只保留眼白与虹膜；DOM 叠加的双瞳孔和双眉根据槽位、轻拨次数与揭晓阶段移动、收窄和下压。`setCatGaze(slot, effort, animate)` 把视线映射为左中右三态（水平 `-7 / 0 / +7px`、垂直 `2–3px`），用 `is-eye-tracking` 重启 260ms 聚焦回弹；`swapAdjacent()` 在每次换位的前 42% 时段让视线在外侧槽与中槽间交替扫动，因此不读取 `flowerCupId`，不会泄露答案。用户试探在输入同步段锁定目标槽，揭晓才锁定答案。390px / 320px 画面分别使用 360px / 310px 宽的等比猫头；`data-phase` 控制 2.8 秒待机呼吸、放置与遮盖的 38–44px 下沉、1.4 秒洗杯跟踪和 1.9 秒选择观察起伏。`is-cat-emerging` 在选择开始时触发 520ms 探头，`is-probing` 在轻拨时额外上探 16px；揭晓继续复用蓄势、侧倾与碰撞姿态。
- 猫爪：同一 DOM 长前腿根据目标槽位切换左右侧，宽 430px、锚点距边缘 354px，常态约 76px、高 56px 留在画内；主体使用深棕色，58×48px 的 `:before` 伪元素在前端形成白手套爪，并同步放大两道爪纹。轻拨目标点深入杯身约 72%，杯子层级遮住接触端，形成从画外连续伸入而非断肢的关系。
...
- 反馈系统：Web Audio 为金属事件叠加 3–4 个非整数倍正弦模态和短带通噪声瞬态，并通过共享 `DynamicsCompressorNode` 控制移动扬声器峰值；轻拨音与触觉延迟 210ms，精确对齐爪尖接触与杯子起转。答案杯获得冲量并进入翻滚的同一同步段为冰花添加 `is-visible is-revealing`，使物品以 90ms 淡入、220ms 生长立即从杯后露出；正确杯第一次碰桌时再显示高对比单轮结果牌、更新分数并播放胜负音。猜中在支持设备上触发 18ms 轻触觉。
